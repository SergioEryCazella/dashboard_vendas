...
## Tabela média de avaliações
avaliacao_estados = dados.groupby('Local da compra')[['Avaliação da compra']].mean().sort_values('Avaliação da compra', ascending=False)
...
fig_avaliacao_estados = px.bar(avaliacao_estados,
                               text_auto=True,
                               title= 'Média das Avaliações por Estado')

fig_avaliacao_estados.update_layout(yaxis_title = 'Média Avaliações')
...
## Visualização stramlit
aba1, aba2, aba3, aba4, aba5 = st.tabs(['Receita', 'Quantidade de Vendas', 'Vendedores', 'Avaliações', 'Desafio'])
...
with aba4:
    st.plotly_chart(fig_avaliacao_estados, use_container_width=True)
